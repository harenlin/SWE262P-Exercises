...
public class ThirtytwoAlphabet {
	...
	private static Map<String, List<Map.Entry<String, Integer>>> regroup(List<List<Map.Entry<String, Integer>>> pairs_list) {
		...
		for(List<Map.Entry<String, Integer>> pairs : pairs_list){
			for(Map.Entry<String, Integer> p : pairs){
				if( p.getKey().length() == 0 || Character.isDigit(p.getKey().charAt(0)) ) continue;
				String startChar = p.getKey().substring(0, 1);
				// System.out.println(startChar);
				if( mapping.containsKey(startChar) ){
					mapping.get(startChar).add(p);
				} else {
					List<Map.Entry<String, Integer>> newList = new ArrayList<>();
					newList.add(p);
					mapping.put(startChar, newList);
				}
			}
		}
		return mapping;
